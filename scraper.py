...
with open('prices.csv', 'w', encoding='UTF8', newline='') as f:
    ...
    for url in urls:
        ...
        if my_attributes[1] == 'voor':
            attributes.append(my_attributes[3])
            # unit_info
            attributes.append(my_attributes[4])
            # price
            attributes.append(my_attributes[my_attributes.index('Voeg toe') - 1])
            # usual_price
            attributes.append(my_attributes[my_attributes.index('Voeg toe') - 1])
        ...
        if my_attributes[0].startswith('VOOR'):
            attributes.append(my_attributes[1])
            # unit_info
            attributes.append(my_attributes[2])
            # price
            attributes.append(my_attributes[0].replace('VOOR ', ''))
            # usual_price
            attributes.append(my_attributes[my_attributes.index('Voeg toe') - 1])
            # discount
            attributes.append(my_attributes[0])
        ...
        if my_attributes[1] != 'korting' and not my_attributes[0].startswith('VOOR') and not my_attributes[1] == 'voor':
            # name
            attributes.append(my_attributes[0])
            # unit_info
            attributes.append(my_attributes[1])
